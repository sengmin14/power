...
public class JwtAuthorizationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        boolean isWhitelisted = WHITELIST_URLS.stream()
                .anyMatch(uri -> requestURI.equals(uri) || requestURI.startsWith(uri + "/"));
        
        if (isWhitelisted || HTTP_METHOD_OPTIONS.equalsIgnoreCase(request.getMethod())) {
        ...
        try {
            ...
            if (StringUtils.isNotBlank(accessToken) || StringUtils.isNotBlank(refreshToken)) {
                ...
                String paramAccessToken = accessToken;
    ...
    /**
     * 클라이언트에게 접근 토큰을 전달합니다. (Cookie로 설정)
     *
     * @param token
     * @param response
     */
    private void sendToClientAccessToken(String token, HttpServletResponse response) {
        ...
        resultMap.put("message", "AccessToken이 갱신되었습니다.");
        try {
            PrintWriter printWriter = response.getWriter();
